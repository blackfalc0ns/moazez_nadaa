...
class DioExceptionHandler {
  static ApiException handleDioException(DioException dioException) {
    ...
    return ApiException(
      ...
      isTranslationKey:
          !isServerMessage, // إذا لم تكن من الـ server، فهي مفتاح ترجمة
    );
  }
  ...
  static String _extractErrorMessage(
    ...
    try {
      ...
      if (_isUnsafePayload(response?.data)) {
      ...
      if (response?.data != null && response!.data is Map<String, dynamic>) {
        ...
        if (nestedError is Map) {
          final nestedMessage = _safeServerMessage(
            nestedError['message'] ?? nestedError['msg'],
          );
        ...
        if (data.containsKey('message') && data['message'] != null) {
          final safeMessage = _safeServerMessage(data['message']);
          if (safeMessage == null) return _getDefaultErrorMessage(errorType);
        ...
        if (data.containsKey('error') && data['error'] != null) {
          final safeMessage = _safeServerMessage(data['error']);
  ...
  static bool _isServerMessage(DioException dioException, String message) {
    try {
      final response = dioException.response;
      if (_isUnsafePayload(response?.data)) return false;
      if (response?.data != null && response!.data is Map<String, dynamic>) {
        final data = response.data as Map<String, dynamic>;
        ...
        if (nestedError is Map) {
          ...
          if (nestedMessage == message) return true;
        }
  ...
  static String? _safeServerMessage(dynamic value) {
    if (value == null || value is Map || value is Iterable) return null;
    final message = value.toString().trim();
    if (message.isEmpty || message.length > 500) return null;
    return _isUnsafePayload(message) ? null : message;
  }

  static bool _isUnsafePayload(dynamic value) {
